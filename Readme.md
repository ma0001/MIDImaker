...
- **ピアノMIDI生成 (`midimaker piano`)**
  ...
  - 周波数リークや音源分離ノイズによる不協和音を防ぐ「半音衝突抑制フィルター」および「極短・微弱ノート除去フィルター」を内蔵。
...
さらに、原曲のリバーブ・エコーやステム分離特有の揺らぎによって発生しやすい**同一キーのマシンガン連打（ゴーストノート）を自動防止する「デバウンスフィルター」**や、周波数リーク・分離アーティファクトによる**半音離れた不協和音ゴーストを自動除去する「半音衝突（短2度）抑制フィルター」**、打鍵ノイズ等のチリチリ音を一掃する**「極短・微弱ノート除去フィルター」**を内蔵。人間が物理的に弾けない超短時間での同一音連続発音や、不自然な半音衝突を自動クリーニングします。
...
# 分離ノイズが多く半音衝突や余分な音が気になる場合の微調整（アタック感度アップ＆フィルター調整）
midimaker piano "stems/piano.wav" --onset-threshold 0.40 --min-duration-ms 50 --min-velocity 30
...
| `--no-semitone-filter` | 無効フラグ | 半音衝突（短2度ゴーストノート）抑制フィルターを無効化 |
| `--clash-window-ms` | `80` | 半音衝突と判定する同時発音アタック時間差ミリ秒 |
| `--min-duration-ms` | `40` | 最小持続ミリ秒。40ms未満などの極短ノイズノートを除去 (0で無効) |
| `--min-velocity` | `25` | 最小ベロシティ。25未満などの微弱ゴースト音を除去 (0で無効) |
